...
import { GenericPojoToGraphNodes } from "./GenericPojoToGraphNodes";

interface VisibilityGraphWidgetProps {
  formData: any;
...
const VisibilityGraphWidget: React.FC<VisibilityGraphWidgetProps> = ({
  formData,
  selectedSystem,
  selectedField,
}) => {
  ...
  // Generate graph data from visibility tree
  useEffect(() => {
    ...
    try {
      ...
      // Create visibility tree for the selected field
      const visibilityTree = TreeUtilities.FsFieldVisibilityGraph.fromFormModel(
        selectedField,
        formModel
      );

      // Log the raw visibility graph data
      const visibilityPojo = visibilityTree.toPojoAt();
      console.log({ visibilityGraph: visibilityPojo });

      // Convert to D3 hierarchy graph nodes
      const d3Nodes = GenericPojoToGraphNodes(visibilityPojo);

      // Log the D3 nodes data
      console.log({ d3Nodes: d3Nodes });

      setGraphData(d3Nodes);
    } catch (error) {
      console.error("Error generating visibility graph:", error);
      setGraphData(null);
    }
  ...
  const getLabelText = (nodeData: any): string => {
    ...
    switch (labelType) {
      ...
      case "value":
        // If node contains a 'value' property, show '{subjectId} {operator} value' on 3 lines
  ...
  if (!selectedSystem || !selectedField) {
    return (
      <Box sx={{ p: 2, textAlign: "center", color: "text.secondary" }}>
        Please select a system and field to view the visibility graph.
      </Box>
    );
  }
...
export default VisibilityGraphWidget;
